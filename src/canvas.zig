...
pub fn getBox(self: *const Canvas, id: []const u8) !*t.Box {
    return self.children.getBox(id);
}

pub fn getText(self: *const Canvas, id: []const u8) !*t.Box {
    return self.children.getText(id);
}
        ...
        for (box.origin.col..(box.origin.col + box.width)) |x| {
            switch (m) {
                ...
                .draw => {
                    ...
                    // TODO: Directly draw the square instead of looping.

                    // WISH I COULD DO THIS INSTEAD 😭
                    // const char: t.Side = switch (x) {
                    //     box.origin.col => {
                    //         switch (y) {
                    //             box.origin.row => .TopLeft,
                    //             box.origin.row + lh => .BottomLeft,
                    //             else => .SideVtl,
                    //         }
                    //     },
                    //     box.origin.col + lw => {
                    //         switch (y) {
                    //             box.origin.row => .TopRight,
                    //             box.origin.row + lh => .BottomRight,
                    //             else => .SideHzn,
                    //         }
                    //     },
                    //     else => .None,
